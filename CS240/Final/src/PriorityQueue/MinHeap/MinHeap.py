...
class MinHeap(list):
    ...
    def heapify(self):
        ...
        for i in range(self.__len__() - 1, 0, -2):
            min_leaf = i if self[i] <= self[i-1] else i-1
            if self.parent(i) > self[min_leaf]:
                self.swap_parent(min_leaf)
    ...
    def delete_min(self):
        ...
        while j <= n:
            ...
            if left_index(j) >= n and self.left(j) < self[j]:
                min_index = left_index(j)

            if right_index(j) >= n and self.right(j) < self[min_index]:
                min_index = right_index(j)
